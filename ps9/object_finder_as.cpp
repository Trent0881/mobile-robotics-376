...
bool ObjectFinder::find_toy_block(float surface_height, geometry_msgs::PoseStamped &object_pose) {
    ...
    //OLD: found_object = pclUtils_.find_plane_fit(0.4, 1, -0.5, 0.5, surface_height + 0.025, surface_height + 0.045, 0.001, plane_normal, plane_dist, major_axis, centroid);
    found_object = pclUtils_.find_plane_fit(0.5, 1, -0.3, 0.3, surface_height + 0.025, surface_height + 0.05, 0.001, plane_normal, plane_dist, major_axis, centroid);
...
void ObjectFinder::executeCB(const actionlib::SimpleActionServer<object_finder::objectFinderAction>::GoalConstPtr& goal) {
    ...
    if (!known_surface_ht) {
        ...
        // OLD: table_ht = pclUtils_.find_table_height(0.0, 1, -0.5, 0.5, 0.6, 1.2, 0.005);
